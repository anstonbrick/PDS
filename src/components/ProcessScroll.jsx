        ...
        <section id="process" ref={sectionRef} className="overflow-hidden bg-render-dark relative z-30">
            <div ref={triggerRef} className="h-screen w-full flex items-center relative overflow-hidden bg-render-dark px-[10vw]">
                ...
                <div className="absolute inset-0 pointer-events-none opacity-20">
                    ...
                    <div className="absolute top-[10%] left-[5%] w-64 h-64 border border-white/5 rounded-full hud-ring flex items-center justify-center will-change-transform">
                ...
                <div className="flex flex-nowrap gap-[10vw] items-center perspective-[2000px]">
                    {steps.map((step, index) => (
                        <div
                            key={index}
                            className={`process-card flex-none w-[85vw] md:w-[35vw] h-[55vh] p-10 bg-render-black/40 backdrop-blur-lg md:backdrop-blur-xl border border-white/5 rounded-3xl flex flex-col justify-between transition-all duration-500 relative overflow-hidden group hover:bg-render-black/60 ${step.border} will-change-transform shadow-2xl`}
                            style={{ contain: 'layout paint style' }}
                        >
                            {/* Inner Glow - Performance: Reduced blur */}
                            <div className={`absolute -bottom-20 -right-20 w-64 h-64 ${step.color} blur-[80px] rounded-full opacity-50 group-hover:opacity-100 transition-opacity duration-700`} />

                            <div className="relative z-10 card-content h-full flex flex-col justify-between">
                                <div>
                                    <div className="flex items-center justify-between mb-12">
                                        <div className={`icon-box p-5 ${step.color} rounded-2xl border border-white/5 backdrop-blur-md group-hover:scale-110 transition-transform duration-500 ease-out`}>
                                            {step.icon}
                                        </div>
                                    ...
                                    <h4 className="text-4xl font-bold text-white mb-6 tracking-tight group-hover:text-electric-blue transition-colors duration-300">
                                        {step.title}
                                    </h4>
                                    <p className="text-gray-400 text-xl leading-relaxed max-w-[90%]">
                                        {step.desc}
                                    </p>
                                </div>

                                <div className="flex items-center gap-4 text-electric-blue group-hover:gap-6 transition-all duration-300">
                                    <span className="text-xs font-bold uppercase tracking-widest">Protocol {step.id}</span>
